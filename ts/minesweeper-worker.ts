...
class MinesweeperSolver {
    ...
    applyRule1(index, neighborInfo, steps) {
        ...
        if (cell.neighborMines !== flaggedNeighbors.length || unrevealedNeighbors.length === 0) return false;
    ...
    tryAdvancedRules(index) {
        ...
        for (let k = 0; k < extendedNeighbors.length; k++) {
            ...
            if (!this.isRevealedNumber(j)) continue;
    ...
    applyAdvancedRules(indexA, indexB) {
        ...
        if (uniqueToA.length === 0 && uniqueToB.length === 0) return null;
    ...
    applyFinalRule() {
        ...
        if (!Number.isFinite(this.totalMines)) return null;
