...
class Minesweeper {
    ...
    countNeighborMines(index) {
        ...
        for (let i = 0; i < neighbors.length; i++) {
            if (this.cells[neighbors[i]].hasMine) count++;
        }
        return count;
    }
    ...
    async applySolvingRulesMainThread(onUpdate, onReveal) {
        ...
        while (changed && iterations < maxIterations) {
            ...
            for (let i = 0; i < this.cells.length; i++) {
                ...
                if (changed) continue; // Skip complex rules if simple rules worked
                ...
                for (let k = 0; k < extendedNeighbors.length; k++) {
                    ...
                    // 開示済みで数字があるセルのみをチェック
                    if (!this.cells[j].isRevealed || this.cells[j].neighborMines === 0) continue;
    ...
    async flushSolvingAnimation(onUpdate) {
        if (!onUpdate) return;
        if (this.animationSpeed > 0 || this.fastFrameStepCount === 0) return;
    ...
    async applyAdvancedRules(indexA, indexB, onUpdate, onReveal) {
        ...
        if (uniqueToA.length === 0 && uniqueToB.length === 0) return false;
    ...
    getRemainingMines() {
        ...
        let flaggedCount = 0;
        for (let i = 0; i < this.cells.length; i++) {
            if (this.cells[i].isFlagged) flaggedCount++;
        }
...
class GameUI {
    ...
    async handleCellClick(index) {
        ...
        if (this.game.isAnimating) return; // Prevent clicking during animation
    ...
    updateComboCounter() {
        if (!this.comboCounter || !this.comboText) return;
